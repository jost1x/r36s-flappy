...
WeatherData WeatherClient::parseResponse(const WeatherLocation& location, const std::string& response) {
    ...
    try {
        ...
        const auto& probabilities = daily.at("precipitation_probability_max");
        const auto& precipitation = daily.at("precipitation_sum");
        if (dates.size() < 5 || codes.size() < 5 || maximums.size() < 5 || minimums.size() < 5 ||
            probabilities.size() < 5 || precipitation.size() < 5)
            throw std::runtime_error("Previsión incompleta");
        result.weatherCode = current.at("weather_code").get<int>();
        result.temperature = rounded(current.at("temperature_2m"));
        result.apparentTemperature = rounded(current.at("apparent_temperature"));
        result.humidity = current.at("relative_humidity_2m").get<int>();
        result.windSpeed = rounded(current.at("wind_speed_10m"));
        result.precipitation = current.value("precipitation", 0.0);
        result.precipitationProbability = probabilities.at(0).get<int>();
        for (size_t index = 0; index < result.daily.size(); ++index)
            result.daily[index] = {dates.at(index).get<std::string>(), codes.at(index).get<int>(),
                                   ...
                                   probabilities.at(index).get<int>(), precipitation.at(index).get<double>()};
        result.updatedAt = nowSeconds();
