...
void WeatherClient::workerLoop() {
    for (;;) {
        PendingRequest pending{};
        {
            ...
            pendingChanged_.wait(lock, [this] { return stopping_ || !pending_.empty(); });
            if (stopping_) return;
...
WeatherData WeatherClient::request(const WeatherLocation& location) {
    ...
    failed.status = WeatherStatus::Error;
    for (int attempt = 0; attempt < 2; ++attempt) {
        CURL* curl = curl_easy_init();
        if (!curl) {
            failed.error = "No se pudo iniciar la red";
            return failed;
        }
        ...
        url << "https://api.open-meteo.com/v1/forecast?latitude=" << location.latitude
            ...
            << "&current=temperature_2m,apparent_temperature,relative_humidity_2m,weather_code,wind_speed_10m,"
               ...
               "precipitation_sum&forecast_days=5&timezone=auto";
        std::string response;
